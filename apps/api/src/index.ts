...
// ─── Socket.IO Server (shared across all namespaces) ───
...
// ─── Code Execution Namespace (/execution) ───

const executionNs = io.of("/execution");

executionNs.on("connection", (clientSocket) => {
  ...
  console.log("✅ Frontend connected to /execution namespace");
  ...
  externalSocket.on("connect", () => {
    console.log("✅ Connected to external execution server");
  });
  ...
  clientSocket.on("disconnect", () => {
    console.log("⚠️ Execution client disconnected");
...
// 1. Socket.IO server (disconnects all sockets across all namespaces)
...
const gracefulShutdown = async () => {
  ...
  try {
    // 1. Close Socket.IO server (sends disconnect to all sockets across /chat and /execution)
    await new Promise<void>((resolve) => {
      io.close(() => {
        console.log("  ✓ Socket.IO server closed (all namespaces)");
        resolve();
      });
    });
...
server.listen(port, () => {
  ...
  console.log(`   Chat:        /chat namespace`);
  console.log(`   Execution:   /execution namespace`);
});
